...
**StopReason** (LLM-15): String enum on `MessageEndEvent` describing why the assistant message ended — `StopReasonStop`, `StopReasonLength`, `StopReasonToolUse`, and `StopReasonUnknown` (the zero value; also where a native finish reason without a portable mapping, e.g. OpenAI's `content_filter`, lands — not an error path, no `LLMErrorEvent` is emitted for it). Mirrors upstream's stop-reason set minus error/aborted, which this API already signals via `LLMErrorEvent`/`StreamResult.Err`. Both providers map their native finish reason onto it; length wins over tool use, since a truncated message's tool-call arguments may be incomplete (upstream #6285) and callers need to be able to tell.
