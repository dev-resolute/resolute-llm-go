...
- **CS-7 (MUST)** - NEVER use `any` as a value type — no `var x any`, no `func f() any`, no `map[string]any`, no struct field `Foo any`. Use explicit return types on exported functions. **Exception:** `any` is permitted as an unconstrained generic type parameter (e.g., `Tool[P any]`, `func Map[T any, U any](...)`) where it expresses parametric polymorphism. This exception does not relax the rule for value-position uses. See ADR-0002.
